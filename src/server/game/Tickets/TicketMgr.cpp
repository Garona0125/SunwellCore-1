...
std::string GmTicket::FormatMessageString(ChatHandler& handler, const char* szClosedName, const char* szAssignedToName, const char* szUnassignedName, const char* szDeletedName, const char* szCompletedName, const char* szViewedName) const
{
    std::stringstream ss;
    ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTGUID, _id);
    ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTNAME, _playerName.c_str());
	...
	if (szCompletedName)
		ss << handler.PGetParseString(LANG_COMMAND_TICKETCOMPLETEDBY, szCompletedName);
	if (szViewedName)
		ss << handler.PGetParseString(LANG_COMMAND_TICKETVIEWEDBY, szViewedName);
    return ss.str();
}
